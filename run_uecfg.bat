...
:parse_args
if "%~1"=="" goto args_done
if /I "%~1"=="--no-pause" set "PAUSE_FLAG=OFF" & shift & goto parse_args
if /I "%~1"=="--pause" set "PAUSE_FLAG=ON" & shift & goto parse_args
...
if "%DEBUG_MODE%"=="1" echo [uecfg] DEBUG mode enabled.
...
set "EXTRA_LOG="
if "%HAS_LOG%"=="0" (
    set "CLI_LOG=%REPO_ROOT%\logs\uecfg_scan_%TS2%.log"
) else (
...
set "CMDLINE=%PY_EXE% -m ue_configurator.cli scan%FORWARD_ARGS%%EXTRA_LOG%"
echo [launcher] cmd: "%PY_EXE%" -m ue_configurator.cli scan%FORWARD_ARGS%%EXTRA_LOG%>>"%LAUNCH_LOG%"
echo [marker] command: "%PY_EXE%" -m ue_configurator.cli scan%FORWARD_ARGS%%EXTRA_LOG%>>"%MARKER%"

call "%PY_EXE%" -m ue_configurator.cli scan %FORWARD_ARGS%%EXTRA_LOG%
set "EXIT_CODE=%ERRORLEVEL%"
set "FINAL_CWD=%CD%"
if not "%EXIT_CODE%"=="0" (
    set "LAST_ERROR=Python scan command failed with exit %EXIT_CODE%."
    goto fail
)
...
echo [uecfg] Completed. Log: %LAUNCH_LOG%
if "%DEBUG_MODE%"=="1" (
    echo [uecfg] DEBUG: Keeping window open.
    cmd /k
...
if defined LAUNCH_LOG (
    echo [launcher] final_cwd: %FINAL_CWD%>>"%LAUNCH_LOG%"
    echo [launcher] exit: %EXIT_CODE%>>"%LAUNCH_LOG%"
    echo [uecfg] Launcher log: %LAUNCH_LOG%
)
echo [uecfg] FAILED (exit %EXIT_CODE%). Marker: %MARKER%
if "%DEBUG_MODE%"=="1" (
    echo [uecfg] DEBUG: Keeping window open after failure.
    cmd /k
) else (
    pause
)
